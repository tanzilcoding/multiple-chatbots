...
try:
    ...
    def generate_response(prompt):
        ...
        return keyword_list, docs_and_scores
    ...
    with container:
        ...
        if submit_button and user_input:
            keyword_list, output = generate_response(
                user_input)
    ...
    if st.session_state['generated']:
        with response_container:
            for i in range(len(st.session_state['generated'])):
                ...
                st.markdown(
                    f"""<h3 style="word-wrap:break-word;">Suggested Keywords:</h3>""", unsafe_allow_html=True)

                counter = 0
                for keyword in keyword_list:
                    counter = counter + 1
                    st.markdown(
                        f"""<span style="word-wrap:break-word;">{counter}. {keyword}""", unsafe_allow_html=True)
